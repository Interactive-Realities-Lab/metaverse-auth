...
namespace VRStandardAssets.Utils
{
    public class UIMovement : MonoBehaviour
    {
        ...
        [SerializeField] private bool m_RotateWithCamera = true;
        [SerializeField] private float m_FollowSpeed = 10f;

        [Range(0.5f, 5f)]
        [SerializeField] private float setDistance = 2f;
        ...
        private void Update()
        {
            ...
            Vector3 targetPosition = m_Camera.position + targetDirection * setDistance;
            targetPosition.y = m_Camera.position.y + heightOffset;

            if (m_RotateWithCamera)
            {
                m_UIElement.position = Vector3.Lerp(
                    m_UIElement.position,
                    targetPosition,
        ...
        private void RefreshCamera()
        {
            ...
            if (mainCam != null)
                m_Camera = mainCam.transform;
        }

        private void SnapToCameraFront()
        {
            if (m_Camera == null || m_UIElement == null)
                return;
